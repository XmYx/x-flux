...
def parse_args():
    parser = argparse.ArgumentParser(description="Training a TemporalLoRA on video frames.")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
...
def main():
    ...
    train_dataloader = loader(**args.data_config)  # Ensure this loader provides pairs of (current_frame, next_frame)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(dit):
                ...
                with torch.no_grad():
                    x_1 = vae.encode(current_frame.to(accelerator.device).to(torch.float32))
                    inp = prepare(t5=t5, clip=clip, img=x_1, prompt=None)  # No textual prompt needed for temporal
                ...
                x_0 = vae.encode(next_frame.to(accelerator.device).to(torch.float32))
                x_t = (1 - t) * x_1 + t * x_0
                bsz = x_1.shape[0]
