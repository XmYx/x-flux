...
class VideoFrameDataset(Dataset):
    def __init__(self, video_folder, transform=None):
        ...
        self.frame_pairs = self.load_frame_pairs()

    def load_frame_pairs(self):
        frame_pairs = []
        ...
        for video_file in video_files:
            ...
            while cap.isOpened():
                ...
                frames.append(frame)
            cap.release()
            # Create pairs of (current_frame, next_frame)
    ...
    def __getitem__(self, idx):
        current_frame, next_frame = self.frame_pairs[idx]
        if self.transform:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Training a temporal ControlNet on video frames.")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
...
def main():
    ...
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Resize((256, 256)),  # Adjust the size according to your model's input
    ])

    train_dataset = VideoFrameDataset(args.video_folder, transform=transform)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                current_frame, next_frame = batch
                current_frame = current_frame.to(accelerator.device)
                next_frame = next_frame.to(accelerator.device)

                with torch.no_grad():
                    x_1 = vae.encode(current_frame.to(torch.float32))
                    inp = prepare(t5=t5, clip=clip, img=x_1, prompt=None)

                    x_1 = rearrange(x_1, "b c (h ph) (w pw) -> b (h w) (c ph pw)", ph=2, pw=2)

                bs = current_frame.shape[0]
                t = torch.sigmoid(torch.randn((bs,), device=accelerator.device))

                x_0 = vae.encode(next_frame.to(torch.float32))
                ...
                block_res_samples = controlnet(
                    ...
                    controlnet_cond=current_frame.to(weight_dtype),
